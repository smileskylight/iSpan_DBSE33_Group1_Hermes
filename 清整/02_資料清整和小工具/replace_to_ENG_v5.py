...
def convert_json_to_eng(file_path, output_folder, city_name):
    city_json = glob.glob(file_path)

    for c in city_json:
        with open(c, "r", encoding="utf-8") as file:
            file_name = os.path.splitext(os.path.basename(c))[0]
            data = json.load(file)
            ENG_json = []
            for line in data:
                ENG_dict = {}
        ...
        output_file_path = os.path.join(output_folder, file_name + "_ENG_v4.json")
        with open(output_file_path, "w", encoding="utf-8") as output_file:
            json.dump(ENG_json, output_file, ensure_ascii=False, indent=2)


convert_json_to_eng(file_path, output_folder, city_name)
